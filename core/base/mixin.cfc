...
<cfcomponent output="false">
	...
	<cffunction name="listLoadedObjectsPropertyNames" access="public" output="false" returntype="string">
		...
		<cfscript>
			...
			var PropertiesMetadata = getLoadedObjectsMetadata().Properties;
		...
		<cfloop collection="#PropertiesMetadata#" item="currentPropertyName">
			...
			<cfloop collection="#FilterByAttributes#" item="currentAttributeName">
				<cfif not existsLoadedObjectsMetadata(currentPropertyName, currentAttributeName)>
				...
				<cfif FindNoCase('*', currentAttributeValue)>
					<cfif not ReFindNoCase('^#ReplaceNoCase(currentAttributeValue, "*", ".*", "all")#$', getLoadedObjectsMetadata(currentPropertyName, currentAttributeName))>
						<cfset IsPropertyTrue = false />
						<cfbreak />
					</cfif>
				<cfelse>
					<cfif not currentAttributeValue is getLoadedObjectsMetadata(currentPropertyName, currentAttributeName)>
						<cfset IsPropertyTrue = false />
						<cfbreak />
					</cfif>					
				</cfif>
